...
export default {
                    ...
                    editor.chasmSubscription = editor.onDidStopChanging((changes) => {
                        for (diff of changes.changes)
                        {
                            ...
                            for (let cursor = diff.start, countY = diff.start.row + diff.newExtent.row; cursor.row <= countY; cursor.row++)
                            {
                                ...
                                {
                                    let type = editor.scopeDescriptorForBufferPosition(cursor).getScopesArray().find((e) => e.includes(".chasm-directive-define.") || e.includes(".chasm-directive-port."));
                                    if (type)
                                    {
                                        let newDefine = true;
    ...
    apiForTokens(lines){
        ...
        for (line of lines)
        {
            ...
            for (token of line)
            {
                ...
                else if (type.includes(".chasm-directive-port.")) kind = KIND.PORT;
    ...
    findDefinesInEditor(editor, grammar){
        ...
        for (port of editor.chasmAPI.ports)
        {
            let marker = editor.markBufferRange(port.range, { invalidate: "inside" });
            port.marker = marker;

            marker.onDidChange(function(port, change){
                ...
                if (api.ports.length != 1)
                {
                    editor.chasmAPI.ports.splice(editor.chasmAPI.ports.indexOf(port), 1);
                    port.marker.destroy();
                }

                else port.name = api.ports[0].name;
            }.bind(this, port));
        }
